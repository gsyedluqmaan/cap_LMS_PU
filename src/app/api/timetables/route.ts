        ...
        for (const slot of timetable.slots) {
          if (slot.teacher.toString() === user._id.toString()) {
            ...
            if (!classSectionMap.has(timetable.classSection._id.toString())) {
              classSectionMap.set(timetable.classSection._id.toString(), timetable.classSection);
            }
          }
        }
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!classSection || !academicYear || !semester || !effectiveFrom || !slots || slots.length === 0) {
      return NextResponse.json(
